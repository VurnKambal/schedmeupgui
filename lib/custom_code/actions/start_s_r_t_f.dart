...
List<ProcessStruct> _sortProcesses(List<ProcessStruct> processes, time) {
  processes.sort((a, b) {
...
Future startSRTF(List<int> arrivalTimeList, List<int> burstTimeList) async {
  int n = 0;
  int time = 0;
  int sumt = 0;
  int sumw = 0;
  ...
  processes = _sortProcesses(processes, time);
  FFAppState().update(() {
  ...
  while (time <= sumBurstTime && completed < n) {
    ...
    if (processes[counter].arrivalTime == time) {
      counter++;
    } else if (processes[0].arrivalTime < time) {
    ...
    if (processes[0].remainingTime <= 0) {
      ...
      print(processes.elementAt(0));
      processes.removeAt(0);

      print("Remove: $processes");
    }
    // print("P\tAT\tBT\tRT");
