...
class ProcessAnimatedListSRTF extends StatefulWidget {
  const ProcessAnimatedListSRTF({super.key, this.width, this.height});
  final double? width;
  final double? height;

  @override
  ProcessAnimatedListSRTFState createState() =>
      ProcessAnimatedListSRTFState(width: width, height: height);
}

class ProcessAnimatedListSRTFState extends State<ProcessAnimatedListSRTF> {
  ProcessAnimatedListSRTFState({this.width, this.height});
  double? width;
  double? height;
...
// Sort processes by arrival time then remaining time
  List<ProcessStruct> _sortProcessesSRTF(List<ProcessStruct> processes, time) {
    processes.sort((a, b) {
...
// Preemptive Shortest Remaining Time First
  Future startSRTF(GlobalKey<ProcessAnimatedListSRTFState>? listKey,
    ...
    int completed = 0;
    int sumBurstTime = 0;
    // List<ProcessStruct> processQueue = [];
    ...
    processes = _sortProcessesSRTF(processes, time);
    FFAppState().update(() {
      FFAppState().newProcessList = List.from(processes);
    });

    print("sorting");
    // print("${listKey.currentState!.processes}");
    // await Future.delayed(Duration(seconds: 1));
    FFAppState().update(() {
      FFAppState().oldProcessList = List.from(processes);
    });

    while (time <= sumBurstTime && completed < n && FFAppState().isSimulating) {
